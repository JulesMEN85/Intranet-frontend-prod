  ...
  const loadWeeklyPlanning = async (week) => {
    try {
      const response = await fetch(
        `http://localhost:4000/api/planning/weekly-planning?startDate=${week}`
      );
...
const handleAddPerson = async (newPerson) => {
  try {
    ...
    const endpoint = "http://localhost:4000/api/planning/with-personnel";
  ...
  const handleSaveEditPerson = async (updatedPerson) => {
    try {
      const response = await fetch(
        `http://localhost:4000/api/planning/base-personnel/${updatedPerson.id}/${selectedWeek}`, // Ajout de la semaine sélectionnée
        {
          method: "PUT",
  ...
  const handleDeletePerson = async (id, semaine) => {
    try {
        const response = await fetch(
            `http://localhost:4000/api/planning/base-personnel/${id}/${semaine}`, // Ajout de la semaine dans l'URL
            {
                method: "DELETE",
            }
        );
