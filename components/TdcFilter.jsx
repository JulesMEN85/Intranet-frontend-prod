...
const baseURL = "http://localhost:4000"; // Remplacez par l'URL de votre API
...
const TdcFilter = ({ setData, setSelectedWeek }) => {
  ...
  useEffect(() => {
    if (date.from && date.to) {
      ...
      if (typeof setSelectedWeek === "function") {
        ...
        console.log("🔍 selectedWeek mis à jour dans TdcFilter:", selectedWeekValue); // Ajoutez ce log
  ...
  const onSubmit = async (event) => {
    ...
    try {
      const url = `${baseURL}/api/planning?startDate=${getDateWithFormatAPI(from)}&endDate=${getDateWithFormatAPI(to)}`;
      ...
      if (typeof setData === "function") {
        setData(data); // ✅ Vérification avant d'appeler setData
      } else {
        console.warn("⚠️ setData n'est pas défini !");
      }
      ...
      if (typeof setData === "function") {
        setData([]); // ✅ Vérification avant de réinitialiser les données
      }
    }
  };
